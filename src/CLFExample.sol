...
contract CLFExample is FunctionsClient, Ownable{
    ...
    event CLFExample_FunctionsRequestSent(bytes32 requestId);
    ///@notice event emitted when functions returns 
    event CLFExample_Response(bytes32 requestId, uint256 returnedValue);
    ///@notice event emitted when an CLF fails
    event CLFExample_RequestFailed(bytes32 requestId, bytes err);

    /*///////////////////////////////////
                Errors
    ///////////////////////////////////*/
    ///@notice error emitted when the requestId is not valid
    error CLFExample_UnexpectedRequestID(bytes32 requestId);
    ///@notice error emitted when a callback tries to fulfill an already fulfilled request
    error CLFExample_RequestAlreadyFulfilled(bytes32 requestId);
    ...
    function sendRequest(
        ...
        emit CLFExample_FunctionsRequestSent(requestId_);
    }
    ...
    function _fulfillRequest(
        ...
        if (request.requestTime == ZERO) revert CLFExample_UnexpectedRequestID(_requestId);
        if (request.isFulfilled) revert CLFExample_RequestAlreadyFulfilled(_requestId);
        ...
        if(_response.length > ZERO){
            ...
            emit CLFExample_Response(_requestId, returnedValue);
        } else {
            emit CLFExample_RequestFailed(_requestId, _err);
        }
    }
